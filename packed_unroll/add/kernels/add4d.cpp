...
void add4d(uint512_dt* A, uint512_dt* B, uint512_dt* out, int* strides_offsets_out, int dim) {
	...
	num_read_outstanding = 2 num_write_outstanding = 1 max_read_burst_length = 64 max_write_burst_length = 64 depth = 16
	#pragma HLS INTERFACE m_axi offset = slave bundle = gmem1 port = B latency = 64 \
	num_read_outstanding = 2 num_write_outstanding = 1 max_read_burst_length = 64 max_write_burst_length = 64 depth = 16
	#pragma HLS INTERFACE m_axi offset = slave bundle = gmem2 port = out latency = 64 \
	num_read_outstanding = 1 num_write_outstanding = 2 max_read_burst_length = 64 max_write_burst_length = 64 depth = 16
	#pragma HLS INTERFACE m_axi offset = slave bundle = plmem0 port = strides_offsets_out latency = 64 num_read_outstanding = 16 num_write_outstanding = 16 max_read_burst_length = 64 max_write_burst_length = 64 depth = 16
	...
	const int l_steps = out_shape[3] + out_end_offset[3];
	const int l_tiles = l_steps / 8; // how many tiles we need to create
