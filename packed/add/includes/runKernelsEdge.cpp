...
void negotiate_strides(	std::vector<int> A_shape, std::vector<int> &out_shape, 
	...
	std::vector<int> A_view_shape, out_view_shape;
	...
	std::vector<int> A_stride, out_stride;
							...
							cl::CommandQueue &q)
{
	...
	// 3*D      + 4d      + D         + D
	// Total length is 8D where D is dimension of output
	int* strides_offsets = aligned_alloc<int>(8 * dimensions + 3 + 3); // we need 8 numbers for each dimension (strides, offsets, output stuff.) then 3 numbers for each array length and three for linear offsets
	...
	strides_offsets[8*dimensions] = A_lin_offset;
	strides_offsets[8*dimensions + 1] = B_lin_offset;
	strides_offsets[8*dimensions + 2] = output_lin_offset;
	strides_offsets[8*dimensions + 3] = A_data_size;
	strides_offsets[8*dimensions + 3 + 1] = B_data_size;
	strides_offsets[8*dimensions + 3 + 2] = output_data_size;

	A_buffer = cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(double) * A_data_size, inputs[0]);
	B_buffer = cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(double) * B_data_size, inputs[1]);
	output_buffer = cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(double) * output_data_size, outputs[0]);
	
	strides_offsets_buffer = cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(int) * (8 + 3 + 3) * dimensions, strides_offsets);
	...
	kernel.setArg(3, strides_offsets_buffer);
	kernel.setArg(4, dimensions);
